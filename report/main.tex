...
\begin{abstract}
This project studies OCR post-correction for historical Turkic texts written in Arabic script. We formulate the problem as a sequence-to-sequence NLP task: given a noisy OCR-like text line, the system should generate its clean version. We collect a small Arabic-script Turkic corpus, construct a synthetic noisy-clean benchmark, compare identity, rule-based, train-derived character-confusion, and ByT5-small approaches, and evaluate the results with Character Error Rate (CER), Word Error Rate (WER), and Exact Match. On the proposed benchmark, ByT5-small with a 512-token sequence length achieves the best CER and WER among the four evaluated methods. The project code is available at: \url{https://github.com/Therad445/low-resource-arabic-script-turkic-ocr}.
\end{abstract}
...
\begin{itemize}
    ...
    \item comparison of identity, rule-based, train-derived character-confusion and neural sequence-to-sequence approaches;
...
\subsection{Train-Derived Character Confusion Baseline}

To add a data-driven non-neural comparison, I also evaluate a train-derived character confusion baseline. The baseline aligns noisy-clean training pairs with Levenshtein dynamic programming and estimates a conservative mapping from noisy characters to clean characters. A mapping is accepted only when the source character has enough support and the most frequent target character is sufficiently dominant. This prevents the baseline from rewriting rare characters based on weak evidence.

This method is intentionally limited. It can substitute or delete existing characters, but it cannot generate a missing character from context and cannot model long-range dependencies. Its role is therefore diagnostic: it tests whether simple corpus-derived character confusions are enough, or whether a contextual sequence-to-sequence model is needed.
...
Table~\ref{tab:compared-methods} summarizes the compared methods. The comparison covers four levels of modelling complexity: no correction, hand-written rule-based correction, data-driven character-level correction and neural sequence-to-sequence correction.
...
All methods are evaluated on the same held-out test split. The split is leakage-safe because it is based on clean source lines rather than noisy variants. The identity, rule-based and train-derived character-confusion baselines are evaluated on the test set. The ByT5-small model is fine-tuned on the training split and evaluated on the same held-out test split.
...
The benchmark includes three non-neural baselines and one neural model. The identity baseline is a conservative reference point. The rule-based normalizer tests whether simple Arabic-script normalization can repair the synthetic OCR-like errors. The train-derived character confusion baseline tests whether corpus-derived character mappings are enough. The ByT5-small model tests whether a byte-level sequence-to-sequence model can learn context-sensitive corrections.
...
The main experimental limitation is that the noisy data is synthetic. This makes the benchmark controlled and reproducible, but it does not fully reproduce real OCR or HTR error distributions. Another limitation is the still modest number of evaluated methods. The current comparison includes identity, hand-written rule-based, train-derived character-confusion and neural sequence-to-sequence approaches, but future work should add more architectures, real OCR outputs, multiple random seeds and more extensive hyperparameter search.
...
Table~\ref{tab:results} reports the final test-set results. The ByT5-small 512-token model achieves the best CER and WER among the evaluated methods on the proposed benchmark. The non-neural baselines are useful diagnostically: the rule-based normalizer tests hand-written normalization, while the train-derived character confusion baseline tests whether corpus-derived character mappings are sufficient.
...
\subsection{Metric-Level Interpretation}

The identity baseline has CER $0.086005$ and WER $0.519006$. This means that the synthetic OCR-like noise is not catastrophic at the character level, but it still damages word-level structure substantially. The rule-based normalizer increases both CER and WER, which suggests that naive normalization changes many characters that should remain unchanged in the clean historical reference. The train-derived character confusion baseline provides an additional data-driven non-neural comparison and shows whether local character mappings alone can close the gap to a contextual neural model.

ByT5-small improves CER from $0.086005$ to $0.079913$ and WER from $0.519006$ to $0.368540$. The WER improvement is stronger than the CER improvement. This suggests that the neural model often restores word-level structure even when it does not exactly recover every character. Exact Match remains low for all methods because the test examples are long: a single remaining character error makes the whole line fail exact matching.

\subsection{Why the Rule-Based Baseline Fails}

The rule-based normalizer is useful as a diagnostic baseline precisely because it is simple. Its worse performance indicates that the benchmark cannot be solved by blindly collapsing Arabic-script variants. In historical Arabic-script Turkic material, character variation may reflect orthographic convention rather than OCR noise. A safe post-correction model must therefore use context and avoid over-normalization.

\subsection{Error Analysis}

Table~\ref{tab:error-summary} compares each ByT5 prediction against the noisy input. The model improves most examples by both CER and WER, but it is not uniformly safe. Some predictions are worse than the original noisy line.
...
This failure pattern is important for practical use. The model should be treated as an assistive post-correction component: it can propose useful corrections, but its output should still be inspected by a human or filtered by confidence/error-detection mechanisms. For scholarly transcription, automatic correction without review would be risky.

\subsection{Qualitative Examples}

In addition to aggregate metrics, we inspected individual examples. Table~\ref{tab:qualitative} shows representative improved and degraded cases. The full Arabic-script noisy, predicted and clean strings are provided in \path{docs/nlp_final/qualitative_examples_full.md}. The examples confirm that ByT5 often repairs local OCR-like distortions, but can also introduce harmful changes.
...
\subsection{SotA Claim on the Proposed Benchmark}

Since this benchmark is newly constructed in the project, there is no previously published result on exactly the same dataset and task. Therefore, the SotA claim is intentionally limited: ByT5-small 512 is the best-performing method among the evaluated approaches on the proposed benchmark, including identity, hand-written rule-based and train-derived character-confusion baselines. The project does not claim global state of the art for OCR post-correction in general.
