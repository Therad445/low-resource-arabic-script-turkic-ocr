...
Построчные примеры показывают локальные типы ошибок, но page-level проверка важна для другого вопроса: что происходит при применении модели к целой странице после разбиения на фрагменты. Ниже приведены несколько характерных случаев из page-level эксперимента. В таблице сравниваются исходный Tesseract OCR, ByT5-small после chunked-применения и strict guarded вариант.

| Случай | Страница | Raw CER | ByT5 CER | Strict CER | Raw WER | ByT5 WER | Strict WER | Интерпретация |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| Небольшое реальное улучшение | 50 | 0.2722 | 0.2542 | 0.2542 | 0.8721 | 0.8219 | 0.8219 | Один из немногих случаев, где ByT5 улучшает и CER, и WER. |
| Улучшение с сохранением вывода ByT5 | 38 | 0.3175 | 0.3011 | 0.3011 | 0.9073 | 0.8488 | 0.8488 | Модель даёт умеренный выигрыш, но масштаб улучшения остаётся небольшим. |
| WER улучшается, но CER ухудшается | 56 | 0.3400 | 0.5157 | 0.3392 | 1.1273 | 0.8970 | 1.1273 | Word-level метрика улучшается, но символный состав портится; strict guard почти возвращает исходный OCR. |
| Сильное ухудшение без фильтрации | 59 | 0.2982 | 0.6945 | 0.2974 | 0.8904 | 0.9726 | 0.8904 | Chunked ByT5 резко ухудшает CER; strict guard предотвращает почти весь ущерб. |
| Guard спасает от неудачной коррекции | 3 | 0.2448 | 0.5176 | 0.2439 | 0.8070 | 0.8363 | 0.8070 | Без фильтрации модель сильно портит страницу, а strict guard фактически оставляет исходный OCR. |

Эти случаи уточняют общий вывод page-level эксперимента. ByT5-small иногда улучшает целую страницу, но таких случаев мало. Гораздо чаще модель, обученная только на синтетическом шуме, изменяет символный состав страницы в худшую сторону. При этом strict guarded вариант полезен как защитный механизм: он не превращает модель в более сильную систему, но снижает риск грубого ухудшения. Поэтому практическое продолжение работы должно опираться не только на синтетический генератор, а на выровненные OCR/reference-фрагменты и обучение на настоящих ошибках OCR.
