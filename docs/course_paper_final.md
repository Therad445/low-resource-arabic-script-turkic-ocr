...
OCR исторических документов отличается от распознавания современных печатных текстов не одной отдельной трудностью, а сочетанием факторов: качеством скана, типографикой, состоянием страницы, разметкой строк, структурой страницы и исторической вариативностью письма. Поэтому в работах по historical OCR задача обычно рассматривается не только как запуск модели распознавания, а как воспроизводимый процесс: подготовка изображения, сегментация, создание ground truth, обучение или настройка OCR/HTR, постобработка и ручная валидация.
